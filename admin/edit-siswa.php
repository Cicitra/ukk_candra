...
include '../koneksi.php';
$nisn = $_GET['nisn'];
$query = mysqli_query($koneksi, "SELECT * FROM siswa,spp,kelas WHERE siswa.id_kelas=kelas.id_kelas
AND siswa.id_spp=spp.id_spp AND nisn");
...
<form method="post" action="?url=proses-edit-siswa&nisn=<?= $nisn; ?>">
    ...
    <div class="form-group mb-2">
        ...
        <option value="<?= $data['id_spp'] ?>"> <?= $data['tahun']; ?> | <?= 
            number_format($data['nominal'], 2, ',','.'); ?> </option>
            <?php
            include '../koneksi.php';
